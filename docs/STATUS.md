...
Siguiente: con `G-021` cerrado, atacar los P1 (`G-023` continuidad del QR, `G-024` coordenadas de despacho, `G-025` contexto de auditoría) y repetir la simulación remota para confirmar `G-022`. Rotar las credenciales expuestas durante el diagnóstico. No entregar accesos a nuevos operadores ni introducir PII hasta que la matriz IDOR y el aislamiento usuario-organización-punto sean verdes también en remoto. Mantener Supabase remoto exclusivamente sintético hasta cerrar G2 y completar/firmar `docs/PILOT_APPROVAL_PACKET.md`.
