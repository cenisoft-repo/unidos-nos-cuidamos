...
La misma simulación abrió dos brechas P0. `G-022` ya cuenta con corrección y pruebas locales; `G-021` quedó cerrado: la ubicación exacta, el contacto privado y el reportante de las necesidades ya no son legibles por el rol autenticado (privilegios por columna), y la verificación autorizada los consulta por `need_case_private_detail`. Quedan pendientes continuidad del QR, coordenadas para despachos reales, contexto/correlación de auditoría y actualización de cortes (`G-023` a `G-027`). Por tanto, el sandbox sirve para corregir y demostrar internamente con datos sintéticos, no para incorporar operadores, PII o actividad real.
...
Siguiente: cerrados los P1 de código (`G-023` continuidad del QR, `G-024` coordenadas moderadas del reporte, `G-025` contexto de auditoría), resta —con autorización explícita— repetir la simulación remota para confirmar `G-022` y avanzar los P2 externos (jurídico, DPIA, financiero, WAF, backups) hacia G2. Rotar las credenciales expuestas durante el diagnóstico. No entregar accesos a nuevos operadores ni introducir PII hasta que la matriz IDOR y el aislamiento usuario-organización-punto sean verdes también en remoto. Mantener Supabase remoto exclusivamente sintético hasta cerrar G2 y completar/firmar `docs/PILOT_APPROVAL_PACKET.md`.
