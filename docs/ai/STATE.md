...
- Próxima acción exacta: en el panel del proyecto, habilitar el registro y la confirmación de correo en Auth, y conceder la autoridad global con `select public.grant_super_admin(...)` desde el editor SQL —sin eso `/registro` no completa y `/operaciones/parametrizacion` no es alcanzable para nadie—. Después rotar la contraseña de la base, que quedó expuesta durante la operación, y repetir el arnés de simulación remota para cerrar `G-022` globalmente.
