...
`scripts/visual-metrics.mjs` recorren **once superficies**: las seis públicas y
las cinco consolas autenticadas, cada una con el rol que la usa —`/operaciones`
dos veces, porque coordinación y aliado ven pantallas distintas—. Las rutas y la
sesión viven en `scripts/lib/rutas-auditadas.mjs`.

Dos reglas del instrumento, para que la evidencia signifique algo:
...
Estado del instrumento: **`audit:a11y` sale en 0** sobre las once superficies, con
3 indeterminados (el pie de la portada, medido a mano: 5,54 · 5,99 · 6,10, todos
por encima de 4,5). `audit:visual` toma 55 mediciones —11 superficies × 5
anchos— sin un solo desborde horizontal.
...
| S-02 | Centro operativo | `/operaciones` | **3** | Auditada con sesión en los dos roles que la usan: contraste en 0, sin desborde de 1440 a 390 px. Falta el recorrido cronometrado con operadores reales, que es validación humana | Prueba de uso con coordinación y con un aliado antes de G2 |
| S-03 | Bodega y logística | `/operaciones/bodega` | **3** | Auditada con sesión de bodega. Aquí apareció DQ-04: el contraste del lote seleccionado. Corregido y verificado | Prueba de uso con bodega antes de G2 |
...
| S-07 | Home pública | `/` | **3** | Validar con usuarios reales antes de G2 | Prueba de comprensión con 5 personas fuera del equipo |
...
Resumen: 1 superficie en nivel 0, 2 en nivel 2, 13 en nivel 3, ninguna en 4.
...
| DQ-03 | Ninguna superficie alcanza nivel 4. No hay microinteracción de estado declarada ni verificación de `prefers-reduced-motion` en los scripts de auditoría | P3 | Abierto |
