...
export default async function TreasuryPage() {
  ...
  const membershipsResult = await supabase.from("memberships").select("role").eq("user_id", user.id).eq("active", true);
  ...
  const results = await Promise.all([
    ...
    supabase.from("financial_transactions").select("id,transaction_type,amount,status,public_reference,created_at").eq("status", "reconciled").order("created_at", { ascending: false }),
    supabase.from("expense_requests").select("id,amount,purpose,status,requested_by,created_at").order("created_at", { ascending: false }),
  ]);
